...
robotAngle = coordinatesTable.getIntegerTopic("angle").publish()
...
while True:
    ...
    if(index != -1):
        ...
        d = (0.035 * f) / xWidth + .20
        print(d)
        cv2.putText(frame,'Distnace: {0:.2f}'.format(d),(30,80),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2,cv2.LINE_AA)


        angle = DEGREES_PER_PIXEL * ( ( (ymax + ymin) / 2 ) - ( 720 / 2 ) )
        cv2.putText(frame,'Angle: {0:.2f}'.format(angle),(30,150),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2,cv2.LINE_AA)

        Y = math.cos(math.radians(angle)) * d
        print(Y)
        cv2.putText(frame,'Y_Distance: {0:.2f}'.format(Y),(30,120),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2,cv2.LINE_AA)
        ...
        heightAboveTable = 1.50 - Y
        robotHeight = heightAboveTable - .33
        cv2.putText(frame,'RobotHeight: {0:.2f}'.format(robotHeight),(30,150),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2,cv2.LINE_AA)

  
        angle = np.arcsin( robotHeight / 0.254 )
        angle = math.degrees(angle)
        cv2.putText(frame,'Angle: {0:.2f}'.format(angle),(30,180),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2,cv2.LINE_AA)
