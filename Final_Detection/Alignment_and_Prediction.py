...
angleTable = ntinst.getTable('Angle')
angle = angleTable.getIntegerTopic("angle").publish()
...
class VideoStream:
    ...
    def __init__(self,resolution=(640,480),framerate=30, cameraNum=0, run=True):
        ...
        ret = self.stream.set(cv2.CAP_PROP_EXPOSURE, -10)  # set the exposure to -4 (corresponds to a shutter speed of 1/1000)
...
videostream_robot = VideoStream(resolution=(imW,imH),framerate=30, cameraNum=0, run=True).start()
videostream_top = VideoStream(resolution=(imW,imH),framerate=30, cameraNum=1, run=True).start()
...
while True:
    ...
    frame_right = videostream_robot.read()
    
    frame_left = videostream_top.read()

    # Acquire frame and resize to expected shape [1xHxWx3]
    frameR1 = frame_right.copy()
    frameR  = cv2.flip(frameR1, 0)
    ...
    if(indexRobot != -1):
        ymin = int(max(1,(boxes_right[indexRobot][0] * imH)))
        xmin = int(max(1,(boxes_right[indexRobot][1] * imW)))
        ymax = int(min(imH,(boxes_right[indexRobot][2] * imH)))
        xmax = int(min(imW,(boxes_right[indexRobot][3] * imW)))

        xWidth = xmax - xmin
        yWidth = ymax - ymin

        cv2.rectangle(frame_right, (xmin,ymin), (xmax,ymax), (0, 165, 255), 2)
    ...
    cv2.rectangle(frameL, (0,  imH // 2 - 100), (imW , imH // 2 + 100), (0, 255, 0), 2) # Draw white line

    rect = [0,  imH // 2 - 100, imW , imH // 2 + 100]

    if(indexTop != -1):
        ymin = int(max(1,(boxes_left[indexTop][0] * imH)))
        xmin = int(max(1,(boxes_left[indexTop][1] * imW)))
        ymax = int(min(imH,(boxes_left[indexTop][2] * imH)))
        xmax = int(min(imW,(boxes_left[indexTop][3] * imW)))

        xWidth = xmax - xmin
        yWidth = ymax - ymin

        xC = int(xmin + xWidth / 2)
        yC = int(ymin + yWidth / 2)
        ...
        if(xC > rect[0] and xC < rect[2] and yC > rect[1] and yC < rect[3]):
            f = 1050
            d = (0.035 * f) / xWidth
            print(d)
            ...
            if(armHeight >= 0):
                theta = math.asin((armHeight)/(.254))
                # angle.set(int(theta))

            foundBallTop = True


        cv2.putText(frameL,'Distance: {0:.2f}'.format(d),(0, 300),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2,cv2.LINE_AA)
        cv2.putText(frameL,'Angle: {0:.2f}'.format(theta),(0, 350),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2,cv2.LINE_AA)
        cv2.putText(frameL,'Height: {0:.2f}'.format(armHeight),(0, 400),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2,cv2.LINE_AA)

        # cv2.putText(frameL,'Angle: {0:.2f}'.format(theta),(0, 400),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2,cv2.LINE_AA)
...
videostream_robot.stop()
videostream_top.stop()
